...
int8_t timerNumbers[HARDWARE_TIMER_DEFINITION_COUNT] = {1, 2, 3, 4};
...
FAST_CODE volatile timCCR_t* timerChCCR(const timerHardware_t *timHw)
{
    ...
    uint8_t cmp_index = ext ? ext->cmp_index : 0;

#ifdef HPM6750
    PWM_Type *ptr = (PWM_Type *)timHw->tim;
    return (volatile timCCR_t*)((volatile char*)&ptr->CMP[cmp_index]);
#else
