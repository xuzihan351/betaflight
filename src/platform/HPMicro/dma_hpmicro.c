...
static FAST_CODE void dmaDispatchIrq(DMA_Type *dma, dmaIdentifier_e first, dmaIdentifier_e last)
{
    ...
    while ((int_stat = dma->INTSTATUS) != 0) {
        ...
        for (dmaIdentifier_e i = first; i <= last; i++) {
            ...
            const uint32_t ch_error = DMA_CHANNEL_IRQ_STATUS_ERROR(ch);
            const uint32_t ch_abort = DMA_CHANNEL_IRQ_STATUS_ABORT(ch);
            const uint32_t ch_tc = DMA_CHANNEL_IRQ_STATUS_TC(ch);
            const uint32_t failureIrqStatus = int_stat & (ch_error | ch_abort);
