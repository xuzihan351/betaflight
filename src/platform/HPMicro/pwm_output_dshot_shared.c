...
static FAST_CODE int combine_edge_data(uint32_t *pos, uint32_t *neg, uint32_t *buf, uint32_t bufSize, uint32_t pos_cnt, uint32_t neg_cnt)
{
    uint32_t posidx = 0, negidx = 0;
    uint32_t idx = 0;
    // Skip all leading zeros in both buffers
    while (posidx < pos_cnt && pos[posidx] == 0) {
        posidx++;
    }
    while (negidx < neg_cnt && neg[negidx] == 0) {
        negidx++;
    }
    ...
    while ((posidx < pos_cnt) || (negidx < neg_cnt)) {
        ...
        const uint32_t posVal = (posidx < pos_cnt) ? pos[posidx] : 0;
        const uint32_t negVal = (negidx < neg_cnt) ? neg[negidx] : 0;
...
FAST_CODE_NOINLINE bool pwmTelemetryDecode(void)
{
    ...
    for (int i = 0; i < dshotMotorCount; i++) {
        ...
        if (dmaMotors[i].isInput) {
            const hpmicroTimerHwExt_t *hw_ext = hpmicroTimerHwExtByTimer(dmaMotors[i].timerHardware);
            int gptmr_idx = trgmDshotGptmrIndexByGptmr(hw_ext ? hw_ext->gptmr : NULL);
            if (gptmr_idx < 0) {
                continue;
            }
            ...
            DMA_Type *dma_base_neg = capNegSpec->ref->base;
            DMA_Type *dma_base_pos = capPosSpec->ref->base;
            uint8_t ch_neg = capNegSpec->ref->channel;
            uint8_t ch_pos = capPosSpec->ref->channel;
            ...
            uint32_t edges_neg = GCR_TELEMETRY_INPUT_LEN - dma_get_remaining_transfer_size(dma_base_neg, ch_neg);
            uint32_t edges_pos = GCR_TELEMETRY_INPUT_LEN - dma_get_remaining_transfer_size(dma_base_pos, ch_pos);
            ...
            if (edges_neg > MIN_GCR_EDGES) {
                ...
                uint32_t len = combine_edge_data(dmaMotors[i].dmaBuffer_pos_edge, dmaMotors[i].dmaBuffer_neg_edge, dmaMotors[i].dmaBuffer, DSHOT_DMA_BUFFER_ALLOC_SIZE, edges_pos, edges_neg);
