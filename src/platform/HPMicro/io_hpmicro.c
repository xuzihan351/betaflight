...
FAST_CODE void IOWrite(IO_t io, bool hi)
{
    ...
    uint32_t ioc_idx = IOCIndex(io);
    gpio_write_pin(HPM_GPIO0, GPIO_GET_PORT_INDEX(ioc_idx), GPIO_GET_PIN_INDEX(ioc_idx), hi);
}
...
FAST_CODE void IOHi(IO_t io)
{
    ...
    uint32_t ioc_idx = IOCIndex(io);
    HPM_GPIO0->DO[GPIO_GET_PORT_INDEX(ioc_idx)].SET = 1U << GPIO_GET_PIN_INDEX(ioc_idx);
}
...
FAST_CODE void IOLo(IO_t io)
{
    ...
    uint32_t ioc_idx = IOCIndex(io);
    HPM_GPIO0->DO[GPIO_GET_PORT_INDEX(ioc_idx)].CLEAR = 1U << GPIO_GET_PIN_INDEX(ioc_idx);
}
...
void IOConfigBPIOC(IO_t io)
{
    ...
    uint32_t ioc_idx = IOCIndex(io);
    if (ioc_idx >= HPM_Y_IOC_BASE && ioc_idx <= HPM_Y_IOC_BASE + 7) {
        HPM_PIOC->PAD[(uint32_t)ioc_idx].FUNC_CTL = 3;
    }
    if (ioc_idx >= HPM_Z_IOC_BASE && ioc_idx <= HPM_Z_IOC_BASE + 7) {
        HPM_BIOC->PAD[(uint32_t)ioc_idx].FUNC_CTL = 3;
    }
}
...
void IOConfigGPIOAF(IO_t io, ioConfig_t cfg, uint32_t af)
{
    ...
    uint32_t ioc_idx = IOCIndex(io);
    const uint32_t port = GPIO_GET_PORT_INDEX(ioc_idx);
    const uint32_t pin  = GPIO_GET_PIN_INDEX(ioc_idx);
    ...
    case IOCFG_AF_PP:
        input = false;
        pad_ctl = HPM_GPIO_OUTPUT_PAD_DEFAULTS;
        break;
    case IOCFG_AF_PP_UP:
        input = false;
        pad_ctl = HPM_GPIO_OUTPUT_PAD_DEFAULTS | IOC_PAD_PAD_CTL_PE_SET(1) | IOC_PAD_PAD_CTL_PS_SET(1);
        break;
    case IOCFG_AF_PP_PD:
        input = false;
        pad_ctl = HPM_GPIO_OUTPUT_PAD_DEFAULTS | IOC_PAD_PAD_CTL_PE_SET(1) | IOC_PAD_PAD_CTL_PS_SET(0);
        break;
    case IOCFG_AF_OD:
        input = false;
        pad_ctl = HPM_GPIO_OUTPUT_PAD_DEFAULTS | IOC_PAD_PAD_CTL_OD_SET(1);
        break;
    case IOCFG_AF_OD_UP:
        input = false;
        pad_ctl = HPM_GPIO_OUTPUT_PAD_DEFAULTS | IOC_PAD_PAD_CTL_PE_SET(1) | IOC_PAD_PAD_CTL_PS_SET(1) | IOC_PAD_PAD_CTL_OD_SET(1);
        break;
    case IOCFG_OUT_PP:
        input = false;
        pad_ctl = HPM_GPIO_OUTPUT_PAD_DEFAULTS;
