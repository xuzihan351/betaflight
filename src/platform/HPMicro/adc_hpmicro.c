...
void adcInit(const adcConfig_t *config)
{
    ...
    for (int i = 0; i < ADC_SOURCE_COUNT; i++) {
        ...
        if ((unsigned)dev >= ADCDEV_COUNT || !adcDevice[dev].ADCx
            || !(adcTagMap[map].devices & (1U << dev))) {
    ...
    for (int dev = 0; dev < ADCDEV_COUNT; dev++) {
        ...
        for (int adcChan = 0; adcChan < ADC_SOURCE_COUNT; adcChan++) {
            ...
            prd_cfg.prescale     = ADC_PERIOD_PRESCALE;
            prd_cfg.period_count = ADC_PERIOD_COUNT;

            const bool configured = adc12_init_channel(adc->ADCx, &ch_cfg) == status_success
                && adc12_set_prd_config(adc->ADCx, &prd_cfg) == status_success;
#elif defined(HPM6360)
            ...
            const bool configured = adc16_init_channel(adc->ADCx, &ch_cfg) == status_success
                && adc16_set_prd_config(adc->ADCx, &prd_cfg) == status_success;
#endif
