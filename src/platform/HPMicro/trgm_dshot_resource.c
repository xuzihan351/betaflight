...
{
    switch (trgm_index) {
    case 0:  return HPM_TRGM0;
    case 1:  return HPM_TRGM1;
#ifdef HPM_TRGM2
    case 2:  return HPM_TRGM2;
#endif
    default: return NULL;
    }
}
...
static bool trgmDshotPwmRefSrcValid(uint32_t pwm_ref_src)
{
    return (pwm_ref_src >= TRGM_DSHOT_PWM_REF_MIN) &&
           (pwm_ref_src <= TRGM_DSHOT_PWM_REF_MAX);
}
...
bool trgmDshotResourceAllocPwmRef(trgmDshotResource_t *res, uint32_t pwm_ref_src)
{
    ...
    if (!trgmDshotPwmRefSrcValid(pwm_ref_src)) {
        return false;
    }
...
bool trgmDshotResourceAllocInput(trgmDshotResource_t *res, GPTMR_Type *gptmr)
{
    ...
    const trgmDshotGptmrMapEntry_t *gptmr_entry = NULL;
    for (uint32_t i = 0; i < TRGM_DSHOT_GPTMR_MAP_COUNT; i++) {
        if (trgmDshotGptmrMap[i].gptmr == gptmr) {
            gptmr_entry = &trgmDshotGptmrMap[i];
            break;
        }
    }
    if (gptmr_entry == NULL) {
    ...
    if (gptmr_entry->trgm_index != res->trgm_index) {
        return false;
    }
    if (allocatedGptmr & (1U << gptmr_entry->gptmr_index)) {
        return false;
    }
    ...
    res->gptmr          = gptmr_entry->gptmr;
    res->gptmr_clock    = gptmr_entry->gptmr_clock;
    res->gptmr_in2_dst  = gptmr_entry->trgm_in2_dst;
    res->gptmr_in3_dst  = gptmr_entry->trgm_in3_dst;

    allocatedGptmr |= (1U << gptmr_entry->gptmr_index);
    return true;
}
