...
FAST_IRQ_HANDLER static void spiRxIrqHandler(dmaChannelDescriptor_t* descriptor)
{
    ...
    if (l1c_dc_is_enabled() && bus->curSegment->u.buffers.rxData) {
        ...
        const uint32_t alignedEnd = HPM_L1C_CACHELINE_ALIGN_UP((uint32_t)bus->curSegment->u.buffers.rxData + bus->curSegment->len);
...
static void spiConfigIOC(const spiDevice_t *spi)
{
    ...
    if (sckIO) {
        uint32_t sck_ioc_idx = IOCIndex(sckIO);
        HPM_IOC->PAD[sck_ioc_idx].FUNC_CTL |= IOC_PAD_FUNC_CTL_LOOP_BACK_SET(1);
    }
}
...
void spiInitDevice(spiDevice_e device)
{
    ...
    const uint32_t spi_clock = clock_get_frequency(spi->rcc);
    ...
    // selected SCLK must divide spi_clock exactly.
    uint32_t divisor = MAX(2U, ((spi_clock + 12000000U - 1U) / 12000000U) & ~1U);
    while ((divisor <= 510U) && ((spi_clock % divisor) != 0U)) {
        divisor += 2U;
    }
    const uint32_t spi_sclk_freq = spi_clock / divisor;
    ...
    if (txData && rxData) {
        ...
        while (len > 0) {
            ...
            stat = spi_transfer(ptr,
                        &control_config,
                        NULL, NULL,
                        (uint8_t *)wbuff, chunk, NULL, chunk);
...
FAST_CODE static hpm_stat_t spi_tx_trigger_dma(DMA_Type *dma_ptr, uint8_t ch_num, SPI_Type *spi_ptr, uint32_t src, bool src_fixed, uint8_t data_width, uint32_t size)
{
...
FAST_CODE static hpm_stat_t spi_rx_trigger_dma(DMA_Type *dma_ptr, uint8_t ch_num, SPI_Type *spi_ptr, uint32_t dst, bool dst_fixed, uint8_t data_width, uint32_t size)
{
    dma_handshake_config_fixed_t config = { 0 };
    config.ch_index = ch_num;
    config.dst = dst;
    config.dst_fixed = dst_fixed;
...
FAST_CODE void spiInternalStartDMA(const extDevice_t *dev)
{
    ...
    if (l1c_dc_is_enabled()) {
        if (txData) {
            ...
            const uint32_t aligned_start = HPM_L1C_CACHELINE_ALIGN_DOWN(txAddr);
            const uint32_t aligned_end = HPM_L1C_CACHELINE_ALIGN_UP(txAddr + bus->curSegment->len);
...
FAST_CODE void spiSequenceStart(const extDevice_t *dev)
{
    ...
    if (dev->busType_u.spi.speed != bus->busType_u.spi.speed) {
        ...
        const uint32_t spi_clock = clock_get_frequency(spiDevice[device].rcc);

        if (hpm_spi_set_sclk_frequency(instance, spi_clock / dev->busType_u.spi.speed) == status_success) {
