...
static bool i2cHandleHardwareFailure(i2cDevice_e device, hpm_stat_t status)
{
    ...
    if (status == status_invalid_argument ||
        status == status_i2c_no_ack ||
        status == status_i2c_no_addr_hit) {
        return false;
    }
...
bool i2cWriteBuffer(i2cDevice_e device, uint8_t addr_, uint8_t reg_, uint8_t len_, uint8_t *data)
{
    ...
    const hpm_stat_t status = reg_ == 0xFF
        ? i2c_master_write(I2Cx, addr_, data, len_)
...
bool i2cReadBuffer(i2cDevice_e device, uint8_t addr_, uint8_t reg_, uint8_t len, uint8_t* buf)
{
    ...
    const hpm_stat_t status = reg_ == 0xFF
        ? i2c_master_read(I2Cx, addr_, buf, len)
...
static void i2c_pin_loopback_enable(IO_t io)
{
    if (!io) {
        return;
    }
...
{
    if (!srcClockHz ||
        clockSpeedKhz < I2C_CLOCKSPEED_MIN_KHZ ||
        clockSpeedKhz > I2C_CLOCKSPEED_MAX_KHZ) {
...
void i2cInit(i2cDevice_e device)
{
    ...
    i2c_pin_loopback_enable(scl);
    i2c_pin_loopback_enable(sda);
