...
static FAST_DATA_ZERO_INIT uart_queue_t uart_queue;
...
static uint32_t gptmr_bidir_baudrate;
...
static bool uart_queue_push(uartDevice_t *uart) {
    uint8_t index = uart_queue.head;
    for (uint8_t i = 0; i < uart_queue.count; i++) {
        if (uart_queue.uart_list[index] == uart) {
            return true;
        }
        index = (index + 1) % (sizeof(uart_queue.uart_list) / sizeof(uart_queue.uart_list[0]));
    }
...
static void uart_queue_remove(uartDevice_t *uart)
{
    const uint8_t pending = uart_queue.count;

    for (uint8_t i = 0; i < pending; i++) {
        uartDevice_t *queuedUart = uart_queue_pop();
        if (queuedUart != uart) {
            uart_queue_push(queuedUart);
        }
    }
}
...
static void timer_config(void)
{
    ...
    if (!slowestBidirBaudrate || (gptmr_configured && slowestBidirBaudrate == gptmr_bidir_baudrate)) {
        return;
    }
    ...
    gptmr_bidir_baudrate = slowestBidirBaudrate;
...
static void uart_schedule_tx_complete(uartDevice_t *uart)
{
    if (!uart_queue_push(uart)) {
        return;
    }
    ...
    gptmr_start_counter(UART_DIR_SWITCH_GPTMR, UART_DIR_SWITCH_GPTMR_CHANNEL);
    intc_m_enable_irq_with_priority(UART_DIR_SWITCH_GPTMR_IRQ,
        hpmPlicPriorityFromNvic(uart->hardware->txPriority));
}

static void enableRxIrq(const uartHardware_t *hardware)
{
    intc_m_enable_irq_with_priority(hardware->irqn,
        hpmPlicPriorityFromNvic(hardware->rxPriority));
}
...
uartPort_t *serialUART(uartDevice_t *uart, uint32_t baudRate, portMode_e mode, portOptions_e options)
{
    ...
    if (options & SERIAL_BIDIR) {
        ...
        if ((mode & MODE_TX) && txIO) {
            uartConfigureBidirPin(txIO, uart->tx.af,
                !serialOptions_pushPull(options), pull);
        }
        if ((mode & MODE_RX) && rxIO) {
...
void uartPinConfigure(const serialPinConfig_t *pSerialPinConfig)
{
    for (size_t hindex = 0; hindex < UARTDEV_COUNT; hindex++) {
        ...
        for (int pindex = 0 ; pindex < UARTHARDWARE_MAX_PINS ; pindex++) {
            if (pSerialPinConfig->ioTagRx[resourceIndex] && (pSerialPinConfig->ioTagRx[resourceIndex] == hardware->rxPins[pindex].pin)) {
                uartdev->rx = hardware->rxPins[pindex];
            }

            if (pSerialPinConfig->ioTagTx[resourceIndex] && (pSerialPinConfig->ioTagTx[resourceIndex] == hardware->txPins[pindex].pin)) {
                uartdev->tx = hardware->txPins[pindex];
            }


            // Check for swapped pins
            if (pSerialPinConfig->ioTagTx[resourceIndex] && (pSerialPinConfig->ioTagTx[resourceIndex] == hardware->rxPins[pindex].pin)) {
                uartdev->tx = hardware->rxPins[pindex];
            }

            if (pSerialPinConfig->ioTagRx[resourceIndex] && (pSerialPinConfig->ioTagRx[resourceIndex] == hardware->txPins[pindex].pin)) {
...
SDK_DECLARE_EXT_ISR_M(UART_DIR_SWITCH_GPTMR_IRQ, tick_ms_isr)
...
void uartEnableTxInterrupt(uartPort_t *uartPort)
{
    ...
    uart_write_byte((UART_Type *)(uartPort->USARTx),
                    uartPort->port.txBuffer[uartPort->port.txBufferTail]);
...
FAST_CODE void uart_isr(uartDevice_t *uart)
{
    ...
    uartPort_t *s = &(uart->port);
    UART_Type *uart_ptr = (UART_Type *)(s->USARTx);

    uint8_t irq_id = uart_get_irq_id(uart_ptr);
    if (irq_id == uart_intr_id_rx_data_avail || irq_id == uart_intr_id_rx_timeout || irq_id == uart_intr_id_rx_line_stat) {
        ...
        while (true) {
            ...
            const uint32_t lineStatus = uart_ptr->LSR;
            ...
            const uint8_t c = uart_read_byte(uart_ptr);
    ...
    if (irq_id == uart_intr_id_tx_slot_avail) {
        if (s->port.txBufferTail != s->port.txBufferHead) {
            ...
            if (s->port.options & (SERIAL_BIDIR | SERIAL_CHECK_TX)) {
                uart_schedule_tx_complete(uart);
            }
        }
    }
}
