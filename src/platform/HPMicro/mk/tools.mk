...
ifeq ($(OSFAMILY)-$(ARCHFAMILY), linux-x86_64)
  ...
  RISCV_DL_CHECKSUM = 3bc1848ac151fcf8792820b0bb851367
...
$(RISCV_INSTALL_MARKER): $(DL_DIR)/$(RISCV_SDK_FILE)
        # verify checksum first (only when one is known for this platform)
	@checksum=$$(md5sum "$<" | awk '{print $$1}'); \
	if [ -n "$(RISCV_DL_CHECKSUM)" ] && [ "$$checksum" != "$(RISCV_DL_CHECKSUM)" ]; then \
...
# Stamp file: use the hpm_sdk submodule root as proof it's populated
