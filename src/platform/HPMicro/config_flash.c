...
FAST_CODE configStreamerResult_e configWriteWord(uintptr_t address, config_streamer_buffer_type_t *buffer)
{
    ...
    if (address % sector_size == 0) {
        status = rom_xpi_nor_erase(BOARD_APP_XPI_NOR_XPI_BASE, xpi_xfer_channel_auto, &s_xpi_nor_config, address - HPMICRO_XPI0_BASE, sector_size);
    ...
    STATIC_ASSERT(CONFIG_STREAMER_BUFFER_SIZE == sizeof(uint32_t) * 8,  "CONFIG_STREAMER_BUFFER_SIZE does not match written size");
    status = rom_xpi_nor_program(BOARD_APP_XPI_NOR_XPI_BASE, xpi_xfer_channel_auto, &s_xpi_nor_config, buffer, address - HPMICRO_XPI0_BASE, CONFIG_STREAMER_BUFFER_SIZE);
...
FAST_CODE void configUnlock(void)
{
    ...
    status = rom_xpi_nor_get_property(base, &s_xpi_nor_config,
                                      xpi_nor_property_total_size, &detectedFlashSize);
    if (status != status_success) {
        configFlashIrqRestore(irq);
        failureMode(FAILURE_FLASH_WRITE_FAILED);
        return;
    ...
    status = rom_xpi_nor_get_property(base, &s_xpi_nor_config,
                                      xpi_nor_property_page_size, &detectedPageSize);
    if (status != status_success) {
        configFlashIrqRestore(irq);
        failureMode(FAILURE_FLASH_WRITE_FAILED);
        return;
    ...
    status = rom_xpi_nor_get_property(base, &s_xpi_nor_config,
                                      xpi_nor_property_sector_size, &detectedSectorSize);
    configFlashIrqRestore(irq);
    if (status != status_success) {
        failureMode(FAILURE_FLASH_WRITE_FAILED);
        return;
    ...
    if (detectedFlashSize == 0 || detectedPageSize == 0 || detectedSectorSize == 0
        ...
        || detectedSectorSize % detectedPageSize != 0
        || detectedFlashSize % detectedSectorSize != 0) {
        failureMode(FAILURE_FLASH_WRITE_FAILED);
        return;
    }
...
/*
 * Returns the XPI NOR configuration built by configUnlock(), for
 * consumers (e.g. the HIL tests) that need to issue their own ROM API
 * reads/erases against the already-configured flash.
 */
FAST_CODE const xpi_nor_config_t *configFlashGetXpiConfig(void)
{
    return &s_xpi_nor_config;
}
